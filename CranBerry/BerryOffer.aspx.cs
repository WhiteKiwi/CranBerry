...
namespace CranBerry
{
    public partial class BerryOffer : System.Web.UI.Page
    {
        ...
        protected void btnWrite_Click(object sender, EventArgs e)
        {
            ...
            cmd.Connection = con;
            cmd.CommandText = string.Format("insert into offer(Title,Content,PostDate,PostIP)values('{0}','{1}','{2}','{3}')", txtTitle.Text, txtContent.Text, DateTime.Now, Request.UserHostAddress);
            con.Open();
            cmd.ExecuteNonQuery();
            con.Close();

            string scriptStr = "<script>alert('제안이 등록되었습니다.'); history.back() ; </script>";
